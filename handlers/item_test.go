...
func TestGetItemByIdHandler(t *testing.T) {
	...
	var test = []module.HandlerTest{
		{Url: "/item?id=1", ExpectCode: http.StatusOK, Expectlenght: 1},
		{Url: "/item?id=4", ExpectCode: http.StatusOK, Expectlenght: 1},
		{Url: "/item?id=7", ExpectCode: http.StatusOK, Expectlenght: 1},
		{Url: "/item?id=abc", ExpectCode: http.StatusBadRequest, Expectlenght: 0},
		{Url: "/item?id=0", ExpectCode: http.StatusNotFound, Expectlenght: 0},
	}
	// передаччя в функцію
	utils.RunHandlerTest(t, GetItemByIdHandler, module.CatalogList, test, http.MethodGet, true)
}
